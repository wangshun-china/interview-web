      ...
      <div class="layout">
        ...
        <main class="content">
          ...
          <section v-if="tab === 'overview'">
            ...
            <div v-if="!dashboard" class="skeleton-overview"><div class="sk-metrics"><span class="sk-pulse"></span><span class="sk-pulse"></span><span class="sk-pulse"></span><span class="sk-pulse"></span></div></div>
          ...
          <section v-if="tab === 'singbox'">
            ...
            <template v-if="sbSubTab === 'traffic'">
              ...
              <h2>活跃连接<button class="ghost" style="margin-left:12px" @click="fetchSbConns">刷新</button></h2>
              <div v-if="!sbConnections?.connections?.length" class="empty">暂无活跃连接</div>
              <div v-else class="table-wrap"><table><thead><tr><th>源</th><th>目标</th><th>协议</th><th>上行/下行</th><th>操作</th></tr></thead><tbody>
                <tr v-for="conn in sbConnections.connections" :key="conn.id"><td>{{ conn.metadata?.sourceIP }}:{{ conn.metadata?.sourcePort }}</td><td>{{ conn.metadata?.destinationIP }}:{{ conn.metadata?.destinationPort }}</td><td>{{ conn.metadata?.network || '-' }}</td><td>{{ ((conn.upload || 0) / 1024).toFixed(1) }}K / {{ ((conn.download || 0) / 1024).toFixed(1) }}K</td><td><button class="link danger" @click="killSbConn(conn.id)">关闭</button></td></tr>
              </tbody></table></div>
            </template>

            <template v-if="sbSubTab === 'config'">
              <h2>出站节点配置</h2>
              <form class="form-grid card form-card" @submit.prevent="saveOutbound">
                <label>标签<input v-model.trim="sbForm.tag" placeholder="jp-01" required /></label>
                <label>协议<select v-model="sbForm.type"><option value="vmess">VMess</option><option value="vless">VLESS</option><option value="trojan">Trojan</option><option value="shadowsocks">Shadowsocks</option><option value="hysteria2">Hysteria2</option><option value="direct">Direct</option><option value="block">Block</option></select></label>
                <label>服务器<input v-model.trim="sbForm.server" placeholder="1.2.3.4" required /></label>
                <label>端口<input v-model.number="sbForm.serverPort" type="number" min="1" max="65535" required /></label>
                <label>策略组<input v-model.trim="sbForm.groupTag" placeholder="proxy" /></label>
                <label>优先级<input v-model.number="sbForm.priority" type="number" /></label>
                <label class="check"><input v-model="sbForm.enabled" type="checkbox" />启用</label>
                <label class="check"><input v-model="sbForm.isDefault" type="checkbox" />设为默认</label>
                <div class="form-actions"><button class="primary" :disabled="busy">{{ sbForm.id ? '保存修改' : '新增节点' }}</button><button v-if="sbForm.id" type="button" class="ghost" @click="resetSbForm">取消</button></div>
              </form>
              <div class="table-wrap"><table><thead><tr><th>标签</th><th>类型</th><th>服务器</th><th>策略组</th><th>操作</th></tr></thead><tbody>
                <tr v-for="o in sbOutbounds" :key="o.id"><td>{{ o.tag }}</td><td>{{ o.type }}</td><td>{{ o.server }}:{{ o.serverPort }}</td><td>{{ o.groupTag }}</td><td><button class="link" @click="editOutbound(o)">编辑</button><button class="link danger" @click="removeOutbound(o)">删除</button></td></tr>
                <tr v-if="!sbOutbounds.length"><td colspan="5" class="empty">暂无节点配置</td></tr>
              </tbody></table></div>
            </template>
          </section>
...
const tabs = [{ key: 'overview', label: '运行概览' }, { key: 'routes', label: '域名绑定' }, { key: 'tunnels', label: '内网穿透' }, { key: 'traffic', label: '流量与任务' }, { key: 'singbox', label: '代理管理' }, { key: 'settings', label: '账号设置' }]
...
const sbStatus = ref<Json | null>(null)
const sbGroups = ref<Json>({})
const sbOutbounds = ref<Json[]>([])
...
async function killSbConn(id: string) { if (!confirm('确定关闭该连接？')) return; beginAction(); try { await api('/singbox/connections/kill', { method: 'POST', body: JSON.stringify({ id }) }); flash('连接已关闭'); await fetchSbConns() } catch (e) { finishError(e) } finally { busy.value = false } }
async function fetchSbTraffic() { try { const data = await api('/singbox/traffic'); sbTraffic.value = data } catch { sbTraffic.value = {} } }
async function saveOutbound() { beginAction(); try { const id = sbForm.id; await api(id ? `/singbox/outbounds/${id}` : '/singbox/outbounds', { method: id ? 'PUT' : 'POST', body: JSON.stringify(sbForm) }); resetSbForm(); await fetchSbStatus(); flash(id ? '节点已更新' : '节点已添加') } catch (e) { finishError(e) } finally { busy.value = false } }
async function removeOutbound(outbound: Json) { if (!confirm(`确定删除节点 ${outbound.tag} 吗？`)) return; beginAction(); try { await api(`/singbox/outbounds/${outbound.id}`, { method: 'DELETE' }); await fetchSbStatus(); flash('节点已删除') } catch (e) { finishError(e) } finally { busy.value = false } }
